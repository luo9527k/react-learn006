...
在React18：不论在什么地方执行setState，它是异步的[都是基于update更新队列机制，实现的批处理]

在React16：如果在合成事件[jsx元素中基于onxxx绑定的事件]、周期函数中，setState的操作都是异步的！！，但是如果setState出现在其他异步操作中[例如：定时器、手动获取DOM元素做的事件绑定]
